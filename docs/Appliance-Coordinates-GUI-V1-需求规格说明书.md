...
- **导入功能**:
  - 支持UTF-8编码（带BOM）
  - 跳过表头行
  - 自动过滤无效数据（坐标格式错误、名称为空）
  - 提供清空现有设备或追加选项
  - 显示导入结果统计（成功数量、跳过数量）
- **导出功能**:
  - 导出当前所有设备到CSV
  - 坐标保留3位小数
  - 默认文件名：`设备列表_YYYYMMDD_HHMMSS.csv`

#### 2.7.4 自动保存功能
- **启用开关**: 默认启用，可在配置中关闭
- **保存间隔**: 默认5分钟，可配置（最小1分钟）
- **草稿文件**: 
  - 保存位置：配置目录/autosave/
  - 文件命名：`draft_YYYYMMDD_HHMMSS.apc`
  - 自动清理：保留最近5个草稿文件
- **恢复提示**: 应用启动时检测最新草稿，询问用户是否恢复
...
#### 2.7.6 配置管理
- **配置文件位置**:
  - Windows: `%APPDATA%\ApplCoord\config.json`
  - macOS: `~/.applcoord/config.json`
  - Linux: `~/.config/applcoord/config.json`
- **配置内容**:
  - 最近打开文件列表
  - 自动保存设置（启用状态、间隔）
  - 用户偏好设置
- **自动创建**: 首次运行自动创建配置目录和默认配置

#### 2.7.7 窗口标题更新
- **格式**: `应用名称 - [项目名称] - 版本标识`
- **修改标记**: 项目修改后标题显示星号 `*`
- **示例**: 
  - 未保存：`家居设备坐标距离角度绘制工具 - [客厅布局 *] - Matplotlib版`
  - 已保存：`家居设备坐标距离角度绘制工具 - [客厅布局] - Matplotlib版`
...
| 属性名 | 类型 | 说明 |
|--------|------|------|
| user_x | float | 用户位置X坐标(世界坐标系) |
| user_y | float | 用户位置Y坐标(世界坐标系) |
| is_set | bool | 用户位置是否已设置 |
| display_mode | string | 坐标显示模式('world'/'dual') |
