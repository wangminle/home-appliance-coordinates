...
├─────────────────────────────┬───────────────────────────────────┤
│                             │                                   │
│     Matplotlib画布区域      │     右侧功能面板 (标签式布局)     │
│        (800x800)            │            (480x800)              │
│                             │                                   │
│  ┌─────────────────────┐   │  ┌─────────────────────────────┐  │
│  │    Figure/Axes      │   │  │ 📍坐标 │🖼️背景 │📋设备 │⚙️操作│  │
│  │    科学绘图区域     │   │  ├─────────────────────────────┤  │
│  │                     │   │  │                             │  │
│  │  - 矢量图形渲染     │   │  │    当前选中标签页的内容      │  │
│  │  - 整数步进网格     │   │  │                             │  │
│  │  - 设备点/信息框    │   │  │  标签页1: 坐标范围/用户坐标  │  │
│  │  - 智能布局避让     │   │  │  标签页2: 背景图导入/设置    │  │
│  │  - 背景户型图       │   │  │  标签页3: 设备列表/CRUD      │  │
│  │  - 交互测量/扇形    │   │  │  标签页4: 导出/重置/快捷键   │  │
│  │  - 用户坐标系覆盖层 │   │  │                             │  │
│  └─────────────────────┘   │  └─────────────────────────────┘  │
└─────────────────────────────┴───────────────────────────────────┘
```
...
- **右侧功能面板**: 采用 `ttk.Notebook` 标签式布局，包含四个功能标签页：
  - **📍 坐标设置**: 坐标范围、用户坐标系、状态指示
  - **🖼️ 背景设置**: 户型图导入、比例设置、透明度
  - **📋 设备管理**: 设备列表、添加/编辑/删除设备
  - **⚙️ 系统操作**: 导出PNG、重置数据、快捷键说明
...
## 4. 右侧功能面板设计 (`input_panel.py`) 🆕 V2.6 标签式布局

### 4.1 整体面板设计

- **总宽度**: 480px
- **背景色**: `#ffffff` (白色)
- **布局方式**: `ttk.Notebook` 标签式布局 🆕 V2.6
- **标签页数量**: 4个
- **默认显示**: 第一个标签页（坐标设置）
...
# 四个标签页
tab_coordinate = ttk.Frame(notebook)  # 📍 坐标设置
tab_background = ttk.Frame(notebook)  # 🖼️ 背景设置
tab_device = ttk.Frame(notebook)      # 📋 设备管理
tab_action = ttk.Frame(notebook)      # ⚙️ 系统操作
```
...
1. **坐标显示范围** (`ttk.LabelFrame`)
   - X/Y轴范围输入框 (`ttk.Entry`, 宽6, 字体Arial 11)
   - 范围设置按钮 (`ttk.Button`)
   - 提示信息标签

2. **用户坐标系** (`ttk.LabelFrame`)
   - 启用开关 (`ttk.Checkbutton`)
   - 用户位置输入 (X/Y坐标 `ttk.Entry`)
   - 设置用户位置按钮

3. **当前状态** (`ttk.LabelFrame`)
   - 坐标系模式标签 (世界/用户坐标系)
   - 用户位置状态标签
   - 交互提示标签
...
1. **图片导入** (`ttk.LabelFrame`)
   - 导入户型图按钮
   - 移除背景按钮

2. **图片信息** (`ttk.LabelFrame`)
   - 显示图片尺寸、DPI信息

3. **比例设置** (`ttk.LabelFrame`)
   - 像素/单位比例输入框
   - 计算结果显示

4. **显示设置** (`ttk.LabelFrame`)
   - 透明度滑块 (`ttk.Scale`, 10%-100%)
   - 显示开关 (`ttk.Checkbutton`)

### 4.5 标签页3: 设备管理 (📋)

**包含两个区域**:

1. **设备列表** (`ttk.LabelFrame`)
   - `ttk.Treeview` 设备列表 (height=12)
   - 列: 设备名称、X坐标、Y坐标、颜色
   - 垂直滚动条

2. **设备信息** (`ttk.LabelFrame`)
   - 名称、X坐标、Y坐标输入框
   - 颜色选择下拉框 (`ttk.Combobox`)
   - 添加/更新、删除按钮
...
1. **导出功能** (`ttk.LabelFrame`)
   - 导出PNG图像按钮
   - 导出说明文本

2. **数据管理** (`ttk.LabelFrame`)
   - 重置所有数据按钮
   - 重置说明文本

3. **快捷键** (`ttk.LabelFrame`)
   - Ctrl/Cmd + S: 导出PNG图像
   - Ctrl/Cmd + R: 重置所有数据

4. **操作提示** (`ttk.LabelFrame`)
   - 左键点击创建测量点
   - 双击绘制90度扇形
   - 右键清除测量点
   - 拖拽标签调整位置
...
1. **标签式布局**: 右侧面板改为 `ttk.Notebook` 标签式布局
2. **四个功能标签页**: 坐标设置、背景设置、设备管理、系统操作
3. **默认显示**: 启动时默认显示"坐标设置"标签页
4. **空间优化**: 减少垂直滚动需求，提升空间利用率
5. **视觉层次提升**: 标签页切换提供更直观的功能导航
6. **快捷键说明**: 新增快捷键说明区域
7. **交互提示增强**: 系统操作标签页新增详细操作提示
8. **代码优化**: input_panel.py 代码优化约25%

### 9.6 V2.7版本更新摘要 🆕

1. **坐标设置标签页背景修复**: 移除 `tk.Canvas` 白色背景包装器，直接使用 `ttk.Frame`
2. **背景色一致性**: 坐标设置标签页现与其他标签页保持相同的浅灰色背景
3. **状态标签同步修复**: 重置后 `coord_mode_label` 和 `user_pos_label` 正确更新
4. **项目加载修复**: 加载项目后用户坐标系五边形正确显示
5. **自动保存完整性**: 背景图、锁定扇形、用户坐标系状态完整保存
6. **重置功能完整性**: 重置后所有状态完全清空，包括背景图、锁定扇形、用户坐标系
7. **Headless环境兼容**: GUI测试支持CI/headless环境自动跳过

**最终评价**: 这是一个技术实现优秀、功能完善、交互流畅的现代化科学计算桌面应用。它成功地将`tkinter`的便捷性与`matplotlib`的专业性结合起来，为特定领域的工程师和设计师提供了高效实用的工具。V2.3版本在标签布局和视觉表现方面进行了重大优化，显著提升了多设备场景下的信息可读性。V2.5版本新增的背景户型图功能，让用户能够在真实户型图上直观地规划设备布局。V2.6版本采用标签式布局重构右侧面板，使界面更加简洁、功能分区更加清晰。V2.7版本通过全面的Bug修复和稳定性提升，确保了应用在各种环境下的可靠运行，进一步提升了用户体验。
